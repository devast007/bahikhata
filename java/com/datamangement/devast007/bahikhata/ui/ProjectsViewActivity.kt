...
class ProjectsViewActivity : AppCompatActivity(), View.OnClickListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        supportActionBar!!.setTitle(R.string.projets)
        getProjects()
    ...
    override fun onClick(view: View?) {
        when (view!!.id) {
            ...
            R.id.tv_project_material -> launchMaterialViewActivity(view)
            R.id.tv_project_transactions -> launchTransactionViewActivity(view, false)
            R.id.tv_payments -> launchTransactionViewActivity(view, true)
            R.id.tv_update_project_info -> updateProjectInfo(view)
            R.id.tv_google_drive_link -> openDriveFolder(view)
        }
    }

    private fun openDriveFolder(view: View) {
        val driveLink: String = view.getTag(R.string.tag_drive_link) as String
        if (TextUtils.isEmpty(driveLink)) {
            Toast.makeText(mContext, "No Link Found", Toast.LENGTH_LONG).show()
            return
        } else {
            var intent = Intent(Intent.ACTION_VIEW, Uri.parse(driveLink))
    ...
    private fun launchMaterialViewActivity(view: View) {
        ...
        intent.putExtra(LedgerDefine.PROJECT_ID, projectID)
    ...
    private fun launchTransactionViewActivity(view: View, isForPayment: Boolean) {
        val projectId: String = view.getTag(R.string.tag_project_id) as String
        ...
        intent.putExtra(LedgerDefine.IS_FOR_PAYMENT, isForPayment)
        startActivity(intent)
    ...
    private fun setSetProject(document: QueryDocumentSnapshot?) {
        if (document != null) {
            ...
            try {
                projectDetails.timeStamp =
                    document.getTimestamp(LedgerDefine.TIME_STAMP)!!.toDate().toString()
            } catch (e: RuntimeException) {
                projectDetails.timeStamp = document.get(LedgerDefine.TIME_STAMP).toString()
            ...
            projectDetails.driveLink = getStringFormDoc(document, LedgerDefine.GOOGLE_DRIVE_LINK)
