...
class UsersViewActivity : AppCompatActivity(), View.OnClickListener {
    ...
    override fun onClick(view: View?) {
        when (view!!.id) {
            R.id.btn_transactions_sent -> launchTransactionViewActivity(
                view,
                LedgerDefine.SENT_TRANSACTION
            )
            R.id.btn_transactions_received -> launchTransactionViewActivity(
                view,
                LedgerDefine.RECEIVED_TRANSACTIONS
            )
            R.id.btn_transactions_sent_master -> launchTransactionViewActivity(
                view,
                LedgerDefine.SENT_TRANSACTION
            )
            R.id.btn_transactions_received_master -> launchTransactionViewActivity(
                view,
                LedgerDefine.RECEIVED_TRANSACTIONS
            )
            R.id.tv_gst -> launchGstViewActivity(view)
            R.id.tv_material -> launchMaterialViewActivity(view)

            R.id.tv_edit_user_info -> modifyUserInfo(view)
            R.id.tv_update_user_info -> updateUserInfo(view)
            R.id.switch_disable_user -> disableUser(view)
            R.id.switch_disable_edit_permission -> changeEditPermission(view)

        }
    }

    private fun changeEditPermission(view: View) {

        val switch: Switch = view as Switch

        var uid: String = view.getTag(R.string.tag_user_id).toString()
        ...
        docRef
            .update(
                LedgerDefine.IS_HAS_EDIT_PERMISSION, switch.isChecked
            )
            .addOnSuccessListener {
                toast(R.string.edit_permission_changed)

                for (user in mUserList) {
                    if (user.userID == uid) {
                        user.isHasEditPermission = switch.isChecked
                        break
                    }
                }
    ...
    private fun disableUser(view: View) {
        ...
        docRef
            .update(
                LedgerDefine.IS_USER_DISABLE, switch.isChecked
            )
            .addOnSuccessListener {
                toast(R.string.user_disable_enable_change)

                for (user in mUserList) {
                    if (user.userID == uid) {
                        user.isUserDisable = switch.isChecked
                        break
                    }
                }
                expandable_list_view!!.deferNotifyDataSetChanged()
    ...
    private fun launchTransactionViewActivity(view: View, type: Int) {
        ...
        intent.putExtra(
            ...
            LedgerDefine.TRANSACTION_VIEW_TYPE_USER
        )

        intent.putExtra(LedgerDefine.ID, userId)
        intent.putExtra(LedgerDefine.TRANSACTION_SENT_OR_RECEIVED, type)
        startActivity(intent)
    }
    ...
    private fun setUsers(document: QueryDocumentSnapshot?) {
        if (document != null) {
            ...
            try {
                userDetails.userCreatedDate =
                    document.getTimestamp(LedgerDefine.TIME_STAMP)!!.toDate().toString()
            } catch (e: RuntimeException) {
                userDetails.userCreatedDate = document.get(LedgerDefine.TIME_STAMP).toString()
            } catch (e: ClassCastException) {
                userDetails.userCreatedDate =
            ...
            temp = document.get(LedgerDefine.IS_USER_DISABLE)
            if (temp != null) userDetails.isUserDisable = temp as Boolean

            temp = document.get(LedgerDefine.IS_HAS_EDIT_PERMISSION)
            if (temp != null) userDetails.isHasEditPermission = temp as Boolean
    ...
    private fun updateData(userID: String, where: String, type: String?) {
        ...
        } else if (type == LedgerDefine.SLASH_MATERIALS_OR_SERVICES) {
                    ...
                    if (queryDocs.isEmpty || size < LIMIT) {
                        ...
                        if (preFix == LedgerDefine.PREFIX_PERSONAL || preFix == LedgerDefine.PREFIX_ADMIN) {
                            if (where == LedgerDefine.SENDER_ID) {
                                ...
                                val masterUserID =
                                    LedgerDefine.PREFIX_MASTER + userID.substring(2)
                                updateData(
                                    masterUserID,
                            ...
                            if (where == LedgerDefine.SENDER_ID) {
                                ...
                                updateData(
                                    personalUserID,
                                    LedgerDefine.SENDER_ID,
                                    LedgerDefine.SLASH_MATERIALS_OR_SERVICES
                                )
                            }
                        }
                    } else {
        ...
        query.get()
            .addOnCompleteListener(OnCompleteListener<QuerySnapshot> { task ->
                if (task.isSuccessful) {
                    ...
                    for (doc: QueryDocumentSnapshot in queryDocs) {
                        val billAmount =
                            (doc.get(LedgerDefine.GST_BILL_AMOUNT) as String).toLong()
